import { Ed25519Keypair, JsonRpcProvider, RawSigner, Connection, RpcTxnDataSerializer } from '@mysten/sui.js';
// types
...
const getSignerFromMnemonic = (provider: JsonRpcProvider, mnemonic: string): RawSigner => {
 const keypair = Ed25519Keypair.deriveKeypair(mnemonic);
 const signer = new RawSigner(keypair, provider);
 return signer;
}
const mnemonic = "giant piano scene gas dance unlock initial brass calm brisk west space"
...
const getSignerFromPrivateKey = (provider: JsonRpcProvider, privKey: Uint8Array): RawSigner => {
...
const constructTransaction = async () => {
    ...
    const serializer = new RpcTxnDataSerializer("https://fullnode.devnet.sui.io:443/");
    const transactionBytes = serializer.serializeToBytes(address, transaction, 'Commit')
...
const execute = async () => {
    ...
    const result = await provider.executeTransaction(trBytes, signature, 'WaitForLocalExecution');
